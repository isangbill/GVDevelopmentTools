...
@interface GVCollectionViewFlowLayout()
...
/**
 *  The itemLayoutAttributes array, to store all the itemLayoutAttributes of UICollectionViewCell in the collectionView.
 */
 ...
 */
@property (nonatomic, assign) UIEdgeInsets currentEdgeInsets;

@end

@implementation GVCollectionViewFlowLayout
...
- (void)prepareLayout {
    ...
    self.itemLayoutAttributes = [[NSMutableArray alloc] initWithCapacity:numberOfSections];
    ...
    for (NSUInteger section = 0; section < numberOfSections; ++section) {
        ...
        [self.itemLayoutAttributes addObject:[NSMutableArray array]];
        [self prepareLayoutInSection:section numberOfItems:itemsInSection];
    }
    
    if ([self.delegate respondsToSelector:@selector(collectionView:didFinishedLayoutWithCollectionLayout:)]) {
        [self.delegate collectionView:self.collectionView didFinishedLayoutWithCollectionLayout:self];
    }
}
...
- (BOOL)shouldInvalidateLayoutForBoundsChange:(CGRect)newBounds {
    ...
    return self.sectionHeadersStickyEnable;
}
...
- (void)prepareLayoutInSection:(NSUInteger)section numberOfItems:(NSUInteger)items {
    ...
    for (NSUInteger colIndex = 0; colIndex < numberOfColumns; ++colIndex)
    ...
    for (NSInteger itemIndex = 0; itemIndex < items; ++itemIndex) {
        NSIndexPath *itemPath = [NSIndexPath indexPathForItem:itemIndex inSection:section];
        NSInteger destColumnIndex = [self preferredColumnIndexInSection:section];
        NSInteger destRowInColumn = [self numberOfItemsForColumn:destColumnIndex inSection:section];
        CGFloat lastItemInColumnOffsetY = [self lastItemOffsetYForColumn:destColumnIndex inSection:section];
         ...
         */
        CGRect itemRect;
        itemRect.origin.x = itemsContentRect.origin.x + destColumnIndex * (interitemSpacing + columnWidth);
        ...
        if ([self.delegate respondsToSelector:@selector(collectionView:layout:heightForItemAtIndexPath:constraintedOnItemWidth:)]) {
            itemRect.size.height = [self.delegate collectionView:collectionView layout:self heightForItemAtIndexPath:itemPath constraintedOnItemWidth:columnWidth];
        } else {
        ...
        [self.itemLayoutAttributes[section] addObject:itemAttributes];
        [self.columnRectsInSection[section][destColumnIndex] addObject:[NSValue valueWithCGRect:itemRect]];
    }
 ...
 */
- (CGFloat)heightOfItemsInSection:(NSUInteger)sectionIndex {
    
    CGFloat maxHeightBetweenColumns = 0.0f;
    NSArray *columnsInSection = self.columnRectsInSection[sectionIndex];
    for (NSUInteger columnIndex = 0; columnIndex < columnsInSection.count; ++columnIndex) {
        CGFloat heightOfColumn = [self lastItemOffsetYForColumn:columnIndex inSection:sectionIndex];
 ...
 */
- (NSInteger)numberOfItemsForColumn:(NSInteger)columnIndex inSection:(NSInteger)sectionIndex {
    return [self.columnRectsInSection[sectionIndex][columnIndex] count];
}
 ...
 */
- (CGFloat)lastItemOffsetYForColumn:(NSInteger)columnIndex inSection:(NSInteger)sectionIndex {
    
    NSArray *itemsInColumn = self.columnRectsInSection[sectionIndex][columnIndex];
    if (itemsInColumn.count == 0) {
        if([self.headerFooterItemAttributes[UICollectionElementKindSectionHeader] count] > sectionIndex) {
            CGRect headerFrame = [self.headerFooterItemAttributes[UICollectionElementKindSectionHeader][sectionIndex] frame];
 ...
 */
- (NSInteger)preferredColumnIndexInSection:(NSInteger)sectionIndex {
    
    NSUInteger shortestColumnIndex = 0;
    CGFloat heightOfShortestColumn = CGFLOAT_MAX;
    for (NSUInteger columnIndex = 0; columnIndex < [self.columnRectsInSection[sectionIndex] count]; ++columnIndex) {
        CGFloat columnHeight = [self lastItemOffsetYForColumn:columnIndex inSection:sectionIndex];
...
- (CGRect)rectForSectionAtIndex:(NSInteger)sectionIndex {
    ...
    if (sectionIndex < 0 || sectionIndex >= self.sectionRects.count)
        return CGRectZero;
    return [self.sectionRects[sectionIndex] CGRectValue];
}
...
- (NSArray *)searchVisibleLayoutAttributesInRect:(CGRect)rect {
    ...
    [visibleSections enumerateIndexesUsingBlock:^(NSUInteger sectionIndex, BOOL *stop) {
        
        // Check item layout attributes's rectangles if intersectes the collectionView's rectangles.
        for (UICollectionViewLayoutAttributes *itemAttr in self.itemLayoutAttributes[sectionIndex]) {
        ...
        if ([self.headerFooterItemAttributes[UICollectionElementKindSectionFooter] count] > sectionIndex) {
            UICollectionViewLayoutAttributes *footerAttribute = self.headerFooterItemAttributes[UICollectionElementKindSectionFooter][sectionIndex];
            ...
            self.currentEdgeInsets = [self.sectionInsetses[sectionIndex] UIEdgeInsetsValue];
        }
        
        // Check header layout attributes's rectangles if intersectes the collectionView's rectangles.
        if([self.headerFooterItemAttributes[UICollectionElementKindSectionHeader] count] > sectionIndex) {
            UICollectionViewLayoutAttributes *headerAttribute = self.headerFooterItemAttributes[UICollectionElementKindSectionHeader][sectionIndex];
            
            if (!self.sectionHeadersStickyEnable) {
...
- (NSIndexSet *)sectionIndexesInRect:(CGRect)rect {
    ...
    for (NSUInteger sectionIndex = 0; sectionIndex < numberOfSections; ++sectionIndex) {
        CGRect sectionRect = [self.sectionRects[sectionIndex] CGRectValue];
        BOOL isVisible = CGRectIntersectsRect(theRect, sectionRect);
        if (isVisible)
            [visibleIndexes addIndex:sectionIndex];
    }
...
- (void)updateHeaderAttributes:(UICollectionViewLayoutAttributes *)attributes
    ...
    CGFloat y = CGRectGetMaxY(currentBounds) - currentBounds.size.height + insetTop;
